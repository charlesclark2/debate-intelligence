...
* `tests/fixtures/debate_files/eval/manifest.json` — committed. Each file by SHA-256, category,
  season, format and template family, and whether it is in the six-file PR subset. **No file name,
  path, school or team code.**
* the **path map** — SHA-256 to local path, which is how the evaluation finds each file again. It
  names schools and team codes, so it is written outside the repository
...
`collect-files` node's manual criterion). Re-running it with the same folders and options
produces the same selection: candidates are ordered by SHA-256, which is stable and has nothing to
do with who wrote a file or what it is called.
...
def _select_category(
    ...
    pool = sorted((c for c in in_category if c.paragraphs <= max_paragraphs), key=lambda c: c.sha256)
...
def _pr_subset(selected: Sequence[Candidate]) -> set[str]:
    ...
    for category in Category:
        ...
        subset.update(c.sha256 for c in picks[:per_category])
    return subset
...
def propose_selection(
    ...
    manifest = Manifest(
        description=(
            "Parser evaluation files, held on the operator's machine and never committed. "
            "Keyed by SHA-256; no file name, school or team code. Proposed by scripts/select_eval_files.py."
        ),
        entries=tuple(c.entry(pr_subset=c.sha256 in subset) for c in selected),
    )
    return manifest, {c.sha256: c.path for c in selected}
