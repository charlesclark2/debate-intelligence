...
| **ac6** prod on `wfbdebate.org` with `www` redirecting, dev on `dev.wfbdebate.org` with noindex, per-environment ACM certificates in us-east-1, DNS validated | **BLOCKED** live; configuration PASS | Configured in `envs/*/terraform.tfvars` and asserted offline by `prod_serves_the_team_domain_and_redirects_www` and `dev_preview_is_not_indexable`. Blocked on the domain: `whois -h whois.publicinterestregistry.org wfbdebate.org` → `Domain not found` (2026-09-20T17:54Z). Every name in ac6 is under `wfbdebate.org`, including the dev preview host |
...
0. **Register `wfbdebate.org`, or find out why its purchase did not complete.** This blocks
   ...
   This is not a propagation delay: the `.org` registry has no record of the name, and a `.com`
   bought eight minutes earlier is already fully delegated. Either the `.org` purchase did not go
   through, or it was made somewhere other than this AWS account. The `route53domains` block in
   runbook step 1 says which. `wfbdebate.org` is the canonical name and is written into
   `envs/prod/terraform.tfvars`, and `dev.wfbdebate.org` sits under it, so **neither environment
   can take its domain until this is resolved** — and an unregistered name someone else can buy is
   worth resolving today.
