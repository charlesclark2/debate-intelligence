...
1. **`prelabel`** parses a file and writes `labels/<sha256>.jsonl` marked `PRELABELED`. It holds
   no text: a unit and a card per paragraph, keyed by index and text hash. The evaluation refuses a
   `PRELABELED` file.
...
| Column | Meaning |
|---|---|
| `index` | The paragraph's position in the body. Do not edit. |
...
## Running it
    ...
    uv run python scripts/prelabel_docx.py worksheet 3561ed9b --out-dir ~/parser-eval-worksheets
    uv run python scripts/prelabel_docx.py import 3561ed9b \\
        --worksheet ~/parser-eval-worksheets/3561ed9b1ff2bbdb.csv --corrected-by coach
    uv run python scripts/prelabel_docx.py mark-reviewed 3561ed9b --reviewer coach

A file is named by any unambiguous prefix of its SHA-256.
"""
...
def is_sampled(file_sha256: str, index: int, fraction: float = SPAN_SAMPLE_FRACTION) -> bool:
    """Whether a paragraph is in the span sample: a stable hash, not a choice anyone makes."""
    digest = hashlib.sha256(f"{file_sha256}:{index}".encode()).digest()
    return int.from_bytes(digest[:4], "big") < fraction * 2**32


def prelabel_document(document: ParsedDocument) -> LabelFile:
    """Turn the parser's reading of a file into labels marked `PRELABELED`.

    Card membership comes from each card's element range; a blank paragraph inside that range is
    not part of the card. Spans are recorded on the sampled non-empty paragraphs.
    """
    ...
    for section in document.sections:
        ...
        if section.text.strip() and is_sampled(document.source_sha256, section.element_index):
            spans.append(
                SpanLabel(
                    index=section.element_index,
    ...
    # A card the parser built but whose paragraphs all fell outside CARD_UNITS has no rows.
...
def write_worksheet(labels: LabelFile, document: ParsedDocument, out_dir: Path) -> Path:
    directory = _outside_repository(out_dir)
    problems = validate_against_texts(labels, document.source_sha256, [s.text for s in document.sections])
    if problems:
    ...
    path = directory / f"{labels.sha256[:16]}.csv"
    with path.open("w", encoding="utf-8-sig", newline="") as handle:
        writer = csv.writer(handle)
        writer.writerow(WORKSHEET_COLUMNS)
        for paragraph, section in zip(labels.paragraphs, document.sections, strict=True):
            ...
            writer.writerow(
                [
                    ...
                    section.text,
                    "" if span is None else render_markup(section.text, span.underline),
                    "" if span is None else render_markup(section.text, span.highlight),
                ]
            )
    return path
...
def import_worksheet(
    ...
    *,
    ...
    if len(rows) != len(texts):
        raise WorksheetError(f"the worksheet has {len(rows)} rows; the file has {len(texts)} paragraphs")
    sampled = {span.index for span in prelabels.spans}
    paragraphs: list[ParagraphLabel] = []
    spans: list[SpanLabel] = []
    completeness: dict[int, CardCompleteness] = {}
    for position, (row, text) in enumerate(zip(rows, texts, strict=True)):
        ...
        if index != position:
            problems.append(
                f"row {line}: rows were reordered or removed (index {index}, expected {position})"
            )
            continue
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prelabel.add_argument("sha", nargs="*", help="SHA-256 prefixes; or --all.")
    ...
    if args.command == "prelabel":
        entries = list(manifest.entries) if args.all else [_resolve(manifest, p) for p in args.sha]
        for entry in entries:
            path = label_path_for(entry.sha256, args.labels_dir)
            if (
                path.exists()
                and not args.force
                ...
                print(
                    f"{entry.sha256[:16]}: already corrected; left alone "
                    "(use --force to discard the correction)"
                )
                continue
            labels = prelabel_document(_document(entry))
            ...
            print(
                f"{entry.sha256[:16]}: {labels.header.paragraph_count} paragraphs, "
                f"{len(labels.cards)} cards, "
                f"{len(labels.spans)} span rows, PRELABELED (parser {DOCX_PARSER_VERSION})"
            )
        return 0

    entry = _resolve(manifest, args.sha)
    path = label_path_for(entry.sha256, args.labels_dir)
    if not path.exists():
        raise SystemExit(f"{entry.sha256[:16]}: no label file; run prelabel first")
    labels = load_label_file(path)

    if args.command == "worksheet":
        written = write_worksheet(labels, _document(entry), args.out_dir)
        print(
            f"{entry.sha256[:16]}: worksheet with {labels.header.paragraph_count} rows "
    ...
    if args.command == "import":
        ...
        try:
            ...
            print(f"{entry.sha256[:16]}: worksheet refused — {error}", file=sys.stderr)
            return 1
        write_label_file(corrected, args.labels_dir)
        print(
            f"{entry.sha256[:16]}: CORRECTED by {args.corrected_by}; "
            f"{corrected.header.rows_changed_from_prelabel} of {corrected.header.paragraph_count} "
            "rows changed "
            "from the pre-labels"
        )
        return 0
    ...
    if labels.header.status is LabelStatus.PRELABELED:
        raise SystemExit(
            f"{entry.sha256[:16]}: still PRELABELED; it has to be corrected before it is reviewed"
        )
    reviewed = replace(
        labels,
    ...
    print(f"{entry.sha256[:16]}: COACH_REVIEWED")
    return 0
