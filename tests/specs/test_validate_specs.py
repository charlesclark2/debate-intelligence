...
class Tree:
    ...
    def errors(self) -> list[str]:
        return validate_specs.collect(self.path).errors

    def tree(self) -> validate_specs.SpecTree:
        return validate_specs.collect(self.path)
...
def test_unparseable_yaml_is_reported_rather_than_raised(tree: Tree) -> None:
    ...
    assert any(e.startswith(prefix) for e in tree.errors())
...
def test_release_file_name_must_be_a_version(tree: Tree) -> None:
    ...
    assert "plan_specs/releases/next.yaml: release file name must be vMAJOR.MINOR.yaml" in tree.errors()
...
def test_major_version_label_must_match_the_folder(tree: Tree) -> None:
    ...
    assert f"{TASK_T01}: debate/major-version label must be 'v1' (folder)" in tree.errors()
...
def test_every_document_in_a_file_carries_identical_labels(tree: Tree) -> None:
    ...
    assert f"{TASK_T01}: every document in a file must carry identical labels" in tree.errors()
...
def test_release_label_must_have_a_release_file(tree: Tree) -> None:
    ...
    assert f"{TASK_T01}: debate/release 'v1.9' has no file in plan_specs/releases/" in tree.errors()
...
def test_release_label_must_belong_to_the_major_version(tree: Tree) -> None:
    ...
    assert f"{TASK_T01}: debate/release 'v2.0' must be a v1 release" in tree.errors()
...
def test_task_name_must_match_its_epic_folder_and_file(tree: Tree) -> None:
    ...
    assert f"{TASK_T01}: task name 'v1-e01-t09-first-step' must start with 'v1-e01-t01-'" in tree.errors()
...
def test_release_label_must_equal_the_release_file_name(tree: Tree) -> None:
    ...
    assert f"{RELEASE_V11}: debate/release label must equal the file name" in tree.errors()
...
def test_plan_graph_must_be_acyclic(tree: Tree) -> None:
    ...
    assert f"{TASK_T02}: Plan graph contains a cycle" in tree.errors()
...
def test_plan_goal_ref_must_name_a_goal_in_the_same_file(tree: Tree) -> None:
    ...
    assert f"{TASK_T01}: Plan goalRef 'release-v1-0' does not match a Goal in the same file" in tree.errors()


def test_gate_nodes_need_a_gate_ref(tree: Tree) -> None:
    tree.edit(RELEASE_V10, "      gateRef:\n        name: release-v1-0-gate\n", "")
    assert f"{RELEASE_V10}: node gate: Gate nodes need gateRef.name" in tree.errors()
...
def test_task_nodes_need_acceptance_criteria(tree: Tree) -> None:
    tree.drop_lines(TASK_T01, "      acceptanceCriteria:", "status:\n  phase: Succeeded")
    assert f"{TASK_T01}: node do-the-thing: Task nodes need acceptanceCriteria" in tree.errors()


def test_criteria_type_must_be_known(tree: Tree) -> None:
    tree.edit(TASK_T01, "      - type: command_succeeds", "      - type: vibes_good")
    assert f"{TASK_T01}: node do-the-thing: unknown criteria type 'vibes_good'" in tree.errors()
...
def test_criteria_need_the_fields_their_type_requires(
    ...
    tree.edit(path, line, "")
    assert f"{path}: node {node}: {ctype} criteria missing ['{missing}']" in tree.errors()


def test_criteria_need_a_name(tree: Tree) -> None:
    tree.edit(TASK_T01, "        name: The thing succeeds\n", "")
    assert f"{TASK_T01}: node do-the-thing: command_succeeds criteria missing ['name']" in tree.errors()
...
def test_an_epic_needs_task_files(tree: Tree) -> None:
    ...
    assert f"{EPIC_E01}: epic has no task files" in tree.errors()
...
def test_a_task_ships_in_its_epics_release(tree: Tree) -> None:
    ...
    assert f"{TASK_T01}: task release v1.1 differs from epic release v1.0" in tree.errors()
...
def test_a_task_file_lives_in_its_epics_folder(tree: Tree) -> None:
    ...
    assert f"{moved}: task file must live in its epic's folder" in tree.errors()
...
def test_an_epic_plan_lists_every_task_in_its_folder(tree: Tree) -> None:
    ...
    assert f"{EPIC_E01}: epic Plan does not list task v1-e01-t02-second-step" in tree.errors()
...
def test_an_epic_plan_lists_no_task_without_a_file(tree: Tree) -> None:
    ...
    assert f"{EPIC_E01}: epic Plan lists v1-e01-t03-ghost-step but no task file declares it" in tree.errors()
...
def test_a_task_epic_label_must_name_a_real_epic(tree: Tree) -> None:
    ...
    assert "plan_specs: tasks reference unknown epic 'v1-e01-ghost-epic'" in tree.errors()
...
def test_a_prerequisite_must_resolve_to_a_task(tree: Tree) -> None:
    ...
    assert f"{TASK_T02}: {expected}" in tree.errors()
...
def test_release_order_forbids_a_prerequisite_from_a_later_release(tree: Tree) -> None:
    ...
    assert f"{TASK_T02}: prerequisite v1-e02-t01-later-step ships in a later release (v1.1)" in tree.errors()
...
def test_the_cross_task_prerequisite_graph_must_be_acyclic(tree: Tree) -> None:
    ...
    assert "plan_specs: cross-task prerequisite graph contains a cycle" in tree.errors()
...
def test_a_release_references_only_epics_that_exist(tree: Tree) -> None:
    ...
    assert f"{RELEASE_V10}: unknown epic 'v1-e09-ghost-epic'" in tree.errors()
...
def test_a_release_references_only_epics_labeled_with_it(tree: Tree) -> None:
    ...
    assert f"{RELEASE_V10}: epic v1-e02-minimal-followup is labeled v1.1" in tree.errors()


def test_every_epic_is_referenced_by_its_release(tree: Tree) -> None:
    tree.edit(RELEASE_V10, "goalRef: v1-e01-minimal-foundation", "goalRef: v1-e09-ghost-epic")
    assert f"{EPIC_E01}: epic is not referenced by release v1.0" in tree.errors()
...
def test_release_order_grows_with_a_new_release_file(tree: Tree) -> None:
    assert tree.tree().release_order == ["v1.0", "v1.1"]
    write_release(tree, "v1.2", "v1-e01-minimal-foundation")
    assert tree.tree().release_order == ["v1.0", "v1.1", "v1.2"]
...
def test_status_counts_phases_per_release_and_per_epic(tree: Tree) -> None:
    lines = validate_specs.render_status(tree.tree())
...
def test_status_says_when_nothing_needs_attention(tree: Tree) -> None:
    assert validate_specs.render_status(tree.tree())[-1] == "No Blocked or Failed tasks."
